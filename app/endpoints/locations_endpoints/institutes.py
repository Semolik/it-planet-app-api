...
async def create_institute(institute: CreateInstitute, db=Depends(get_async_session),
    ...
    city = await LocationsCrud(db).get_city(city_id=institute.city_id)
    if not city:
        raise HTTPException(404, "City not found")
    if institute.university_id:
        university = await LocationsCrud(db).get_university(
            university_id=institute.university_id)
        if not university:
            raise HTTPException(404, "University not found")
    city = await LocationsCrud(db).create_institute(name=institute.name, city_id=institute.city_id, university_id=institute.university_id)
    return await LocationsCrud(db).get_intstitute(institute_id=city.id)
...
async def get_institutes(city_id: uuid.UUID, page: int = Query(1, ge=1), db=Depends(get_async_session)):
    return await LocationsCrud(db).get_institutes(city_id=city_id, page=page)
...
async def search_institutes(query: str = None, page: int = Query(1, ge=1), db=Depends(get_async_session)):
    return await LocationsCrud(db).search_institutes(query=query, page=page)
...
async def get_institute(institute_id: uuid.UUID, db=Depends(get_async_session)):
    institute = await LocationsCrud(db).get_intstitute(institute_id=institute_id)
    if not institute:
...
async def update_institute(institute_id: uuid.UUID, institute: CreateInstitute, db=Depends(get_async_session),
                           current_user: User = Depends(current_superuser)):
    db_institute = await LocationsCrud(db).get_intstitute(institute_id=institute_id)
    if not db_institute:
        raise HTTPException(404, "Institute not found")
    city = await LocationsCrud(db).get_city(city_id=institute.city_id)
    if not city:
    ...
    if institute.university_id:
        ...
        if not university:
            raise HTTPException(404, "University not found")
    return await LocationsCrud(db).update_institute(institute=db_institute, name=institute.name, city_id=institute.city_id, university_id=institute.university_id)
...
async def delete_institute(institute_id: uuid.UUID, db=Depends(get_async_session),
    ...
    if not db_institute:
        raise HTTPException(404, "Institute not found")
    await LocationsCrud(db).delete(db_institute)
