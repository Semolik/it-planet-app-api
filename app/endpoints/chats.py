...
async def create_chat(user_id: UUID = Query(..., description='ID пользователя, с которым создается чат'), db=Depends(get_async_session), current_user=Depends(current_active_user)):
    ...
    db_chat = await ChatsCrud(db).create_chat(user_id_1=current_user.id, user_id_2=user_id)
    return Chat(**db_chat.__dict__)
